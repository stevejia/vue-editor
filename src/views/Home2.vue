  ...
  methods: {
    ...
    mouseUp(event) {
      ...
      vueLoader(cContent).then(comp => {
        ...
        let axisContainer = document
          .createRange()
          .createContextualFragment(`<div id="mount-area"></div>`);
        instance.$el.appendChild(axisContainer);
        let AxisComponent = Vue.extend(CompAxis);
        if (this.axisVM) {
          this.axisVM.$destroy();
          this.axisVM.$el.remove();
          this.axisVM = null;
          document.removeEventListener("keydown", this.compKeyDown, false);
        }
        let axisVM = new AxisComponent().$mount("#mount-area");
        this.axisVM = axisVM;
        axisVM.$el.setAttribute("comp-id", id);
        this.attrData = this.compData[id].$data;
        this.compId = id;
    ...
    compKeyDown(e) {
      switch (e.keyCode) {
        ...
        case 40:
          ...
          break;
        case 46:
          let compVm = this.compData[this.compId];
          compVm.$destroy();
          compVm.$el.remove();
          compVm = null;
          delete this.compData[this.compId];
